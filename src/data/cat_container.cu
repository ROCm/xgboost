...
namespace cuda_impl {
  ...
  #define MakePair = cuda::std::make_pair;
  #define Visit = cuda::std::visit;
  #define Get = cuda::std::get;
#elif defined(XGBOOST_USE_HIP)
  using Pair = std::pair<std::int32_t, std::int32_t>;
  using ProclaimReturnType = Pair;
  // For HIP, use macros to directly call std:: functions
    ...
    for (std::size_t f_idx = 0, n = that.columns.size(); f_idx < n; ++f_idx) {
      ...
      auto dispatch = enc::Overloaded{
          [this, f_idx, &h_columns_v, stream](enc::CatStrArrayView const& str) {
            ...
            auto& col_v = Get<enc::CatStrArrayView>(h_columns_v[f_idx]);
          ...
          [this, f_idx, &h_columns_v, stream](auto&& values) {
            ...
            auto& col = Get<dh::device_vector<T>>(this->columns[f_idx]);
