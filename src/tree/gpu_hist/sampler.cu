...
#include "../../common/linalg_op.cuh"  // for linalg::tbegin, tend, tcbegin, tcend
...
struct GradReduceOp {
  ...
  MvsGradOp grad_op;
  ToFloat to_float;
  XGBOOST_DEVICE float operator()(thrust::tuple<std::size_t, GPair> const& tup) const {
    return grad_op(to_float(thrust::get<0>(tup), thrust::get<1>(tup)));
  }
};
#endif
...
#if defined(XGBOOST_USE_HIP)
  GradReduceOp<GPair, std::decay_t<ToFloat>> op{grad_op, to_float};
#else
...
std::size_t CalculateThresholdIndex(Context const* ctx, common::Span<float> sorted_rag,
  ...
  return
#if defined(XGBOOST_USE_HIP)
      std::distance(dh::tbegin(grad_csum), min) + 1;
#else
      cuda::std::distance(dh::tbegin(grad_csum), min) + 1;
#endif
}
...
void GradientBasedSampling::Sample(Context const* ctx, linalg::MatrixView<GradientPairInt64> gpair,
  ...
  thrust::transform(cuctx->CTP(), reg_abs_grad_.cbegin(), reg_abs_grad_.cend(),
                    reg_abs_grad_.begin(),
                    [] XGBOOST_DEVICE(float gpair) {
#if defined(XGBOOST_USE_HIP)
        return std::sqrt(gpair);
#else
        return cuda::std::sqrt(gpair);
#endif
      });
