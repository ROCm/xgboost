...
struct ScanHistogramAgent {
  ...
  __device__ void ScanFeature(GradientPairInt64 const *node_histogram,
    ...
    auto lane_id = static_cast<std::int32_t>(threadIdx.x % dh::WarpThreads());
    ...
    for (auto scan_begin = gidx_begin; scan_begin < gidx_end; scan_begin += dh::WarpThreads()) {
      ...
      WarpScanT(*tmp_storage).InclusiveScan(bin, bin,
#if defined(XGBOOST_USE_HIP)
      std::plus<>(),
#else
      cuda::std::plus{},
...
#if defined(XGBOOST_USE_HIP)
                      [](bst_bin_t bin_idx) { return bin_idx; });
#else
                      cuda::std::identity{});
#endif
  }
...
struct EvaluateSplitAgent {
  ...
  __device__ void Numerical(MultiEvaluateSplitInputs const &node,
    ...
    for (auto scan_begin = gidx_begin; scan_begin < gidx_end; scan_begin += dh::WarpThreads()) {
      ...
      auto best_thread = __shfl_sync(
#if defined(XGBOOST_USE_HIP)
          0xffffffffffffffffULL,  // 64-bit mask required for HIP (e.g. MI300 64-wide wavefront)
#else
          0xffffffffu,
#endif
          best.key, 0);
...
void MultiHistEvaluator::ApplyTreeSplit(Context const *ctx, RegTree const *p_tree,
  ...
  auto max_node = thrust::reduce(
      ...
      [=] XGBOOST_DEVICE(bst_node_t l, bst_node_t r) {
      return
#if defined(XGBOOST_USE_HIP)
          std::max(l, r);
#else
