...
auto ProclaimReturn(F&& f) { return cuda::proclaim_return_type<T>(std::forward<F>(f)); }
#elif defined(XGBOOST_USE_HIP)
...
using TreeViewVar = gpu_std::variant<tree::ScalarTreeView, tree::MultiTargetTreeView>;
...
void ExtractPaths(Context const* ctx,
  ...
  auto end = thrust::copy_if(
      ...
      ProclaimReturn<bool>([=] __device__(PathInfo const& e) { return e.IsLeaf(); }));
  ...
  auto length_iterator = dh::MakeTransformIterator<LenT>(
      info.begin(), ProclaimReturn<LenT>(
