...
template <bool has_categorical, typename TreeView,
          std::enable_if_t<!std::is_same_v<std::remove_cv_t<std::remove_reference_t<TreeView>>, RegTree::Node>>* = nullptr>
...
// Overloads for RegTree::Node (node-based API)
...
XGBOOST_DEVICE bst_node_t GetNextNode(RegTree::Node const& node, bst_node_t nid, float fvalue,
                                      bool is_missing,
                                      RegTree::CategoricalSplitMatrix const& cats) {
  (void)nid;
  if (has_missing && is_missing) {
